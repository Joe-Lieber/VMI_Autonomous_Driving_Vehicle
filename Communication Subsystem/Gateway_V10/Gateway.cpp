...
//[Target Heading(16 bits), Empty(2 bit), Emergency Reset(1 bit), Mode Select(1 bit)]
  ...
  void serialHandle() {
    if (Serial.available()) {
      ...
      if (input == 'w') {
        strcpy(steering_packet, "0A001");
        strcpy(throttle_braking_packet, "010017D1");
        sendEthernet();
        Serial.println(".");
      } 
        ...
        strcpy(steering_packet, "0A001");
        strcpy(throttle_braking_packet, "00FF0001");
        sendEthernet();
        Serial.println(".");
        ...
        strcpy(steering_packet, "0FA01");
        strcpy(throttle_braking_packet, "010013C1");
        sendEthernet();
        Serial.println(".");
      }

      else if (input == 'a') {
        strcpy(steering_packet, "03E81");
        strcpy(throttle_braking_packet, "010013C1");
        sendEthernet();
        Serial.println(".");
