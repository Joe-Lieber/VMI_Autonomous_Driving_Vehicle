...
const int MAX_TORQUE = 400;      // The threshold for the difference of the two torque value before EMG is thrown
// Encoder Values
...
const int ADJUSTMENT_SPEED = 40; // The minimum PWM value sent to the motor
...
//Timers
const int TURN_PERIOD = 10;      // turn period, time in [mS] before the turn speed can increment up or down
...
void initialize() {
  ...
  // TCCR1A |= 0x03;                    //****** Implement higher frequency PWM
  // TCCR1B &= ~_BV(WGM12);             //****** Implement higher frequency PWM
  // TCCR1B |= 1<<CS00;                 //****** Implement higher frequency PWM
  // TCCR1B &= ~0x60;                   //****** Implement higher frequency PWM
  //  OCR1A = 150;                      //****** Implement higher frequency PWM
  }
...
void readSensors() {
  ...
  if (torque_a - torque_b > MAX_TORQUE || torque_b - torque_a > MAX_TORQUE) {  // If the difference of the values is larger than the threshold
    EMG = true;                                                                // Set the Emergency stop flag
  }
